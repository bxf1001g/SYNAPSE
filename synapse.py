"""
synapse.py — SYNAPSE Launcher / Supervisor
...
Usage:
    python synapse.py [--workspace ./myproject] [--port 8080]
...
BANNER = f"""
{C_PURPLE}{C_BOLD}  ╔══════════════════════════════════════════════╗
  ║  ◈  SYNAPSE — Self-Evolving AI System         ║
...
class SelfModEngine:
    ...
    def __init__(self, base_dir, port):
        ...
        self.port = port
        self.backup_dir = self.base_dir / ".synapse_backups"
    ...
    def process_request(self, request_data):
        ...
        log_ok(f"Backups saved to .synapse_backups/{timestamp}/")
...
class SynapseSupervisor:
    ...
    def start(self):
        ...
        if not self.api_key:
            log_warn("No GEMINI_API_KEY set — you can configure keys in the web UI")
        ...
        try:
            ...
            self._stop_agent()
            log_info("SYNAPSE supervisor stopped")
...
def main():
    parser = argparse.ArgumentParser(
        description="SYNAPSE — Self-Evolving AI System Launcher",
...
Examples:
    python synapse.py --workspace ./myproject
    python synapse.py --port 9000 --model gemini-2.0-flash
""",
    )
    ...
    supervisor = SynapseSupervisor(args)
    supervisor.start()
